...
and reflectance peak ~700 nm from min sum of total absorb.
...
Measured reflectance spectra need to be corrected for air-water interface
and normalization of $R^{+}_{rs}$ to zenith illumination and viewing
geometry to get **normalized water-leaving reflectance**: $[R_w]_N$

# Reflectance Normalization

> TODO: rescale Rrs based on zenith angles 

> TODO: investigate using PAR above and below 
surface for refining surface factor
...
This is a whole thing. Based on idea that residual radiance in NIR 
(~700 - 900 nm) is from glint
...
of image/sample, and calculate deglinted Rrs via: 



* Kutser et al 2009 assume glint at 760 nm, and the depth of the absorption
feature at 760 nm is proportional to glint:
...
$a_{tot}(\lambda) = a_{w}(\lambda) + a_{CDOM}(\lambda) + a_{phy}(\lambda) + a_{SM}(\lambda)$
...
(**Grunert et al 2018**) especially based on lignin absorbance ~ 276 nm
...
$\gamma_{bb}$ is the power law exponent of particulate backscattering coefficient, can be related to the size of particles. low values (~0.3)
for larger particles and higher values (~1.7) for smaller particles. 
...
Get parameters for each site (slope factors, etc.)

Input spectra --> fitted model with site parameters  --> estimate CDOM absorbance

Extract spectra from broader area --> estimate CDOM absorbance
