...
* **Remote sensing reflectance** ($sr^-1$) uses upwelling radiance instead of upwelling irradiance and is defined as:
...
* $R_{rs}(0+)$ is remote sensing reflectance just above water surface

* $R_{rs}(0-)$ or $r_{rs}$ is remote sensing reflectance just below water surface

The flux contained within a small solid angle spreads out to a larger solid angle so the value of emergent radiance at any angle is approx 55% of the corresponding subsurface radiance (Kutser et al 2001). Austin 1980 proposed the factor **0.544** for relating radiance just above the surface with radiance just below the surface (including an effect for internal reflection). 

Kutser model (for diffuse component of remote sensing reflectance just above water surface): 

$$R_D(\lambda) = 0.544 \times (0.629 \mu_0 + 0.975) \bigg(\frac{b_b(\lambda)}{a(\lambda) + b_b(\lambda)}\bigg) $$
Or:

$$ r_{rs}(\lambda, 0^-) = (0.629 \mu_0 + 0.975) \bigg(\frac{b_b(\lambda)}{a(\lambda) + b_b(\lambda)}\bigg) $$
and $$R_{rs}(\lambda, 0^+) = 0.544\times r_{rs}(\lambda, 0^-)$$

Air-sea interface transfer coefficients from Lee et al 1999:

$$R_{rs}(0+) = \frac{0.52 \times R_{rs}(0-)}{1-1.7 \times R_{rs}(0-)}$$

**Gordon et al 1975, 1998** model for remote sensing reflectance just below the water surface based on Monte Carlo simulations. See Lee et al 2002 for some discussion of g1 and g2. Lee et al 2002 discuss getting total absorption. 
...
g1 or *C* (ignoring other terms in the polynomial) depends on solar zenith angle (0.32 for sun at zenith, or 0.33 according to Prieur 1976). Kirk 1984 found *C* to be a function of the cosine of the zenith angle of refracted photons ($\mu_0$)

$$C(\mu_0) = -0.629\mu_0 + 0.975$$

Reflectance *decreases* as the solar altitude increase (solar altitude angle + solar altitude angle = 90&deg;). For sun at zenith ($\theta_{sun} = 0$) this relationship predicts C = 0.346 (Kirk 1994). 

```{r,eval=TRUE}
data.frame(theta_sun = seq(-pi/2, pi/2, 0.1)) %>%
  mutate(mu0 = cos(theta_sun)) %>%
  mutate(theta_sun_deg = theta_sun * 180/pi) %>%
  mutate(Cmu0 = -0.629 * mu0 + 0.975) %>%
  ggplot(aes(x = theta_sun_deg, y = Cmu0)) +
  geom_vline(aes(xintercept = 0), lty = 2) +
  geom_hline(aes(yintercept = 0.346), lty = 2) +
  geom_line() +
  # geom_point() +
  ylab(expression(C~(mu[0]))) +
  xlab(expression(theta[sun]~deg)) +
  ylim(c(0, NA)) +
  theme_minimal()
```
...
* Shape of reflectance spectra could be qualitatively compared to optical water typologies proposed by **Spyrakos et al 2018 L&O**

* Chl retrieval algorithms make use of chl absorption peaks at 442, 665, and reflectance peak around 700 nm from min sum of total absorb.
...
> TODO: investigate using PAR and uPAR (above and below surface)
...
* Potential for more statistical based approaches (neural networks) for high res applications
...
The reflectance value of interest for aquatic purposes is the ratio of water leaving radiance to downwelling planar irradiance:
...
* Sun glint can cause false positives in fire detection algorithms for MODIS (Giglio et al 2003). Channels at 0.65, 0.86, 2.1 (1, 2, 7) $\mu$m are used to test for false positives, along with the angle between surface-to-satellite and specular reflection directions and number of adjacent water pixels.  
...
* chlorophyll a is an indicator of photosynthetic plankton biomass and a proxy for phytoplankton productivity. 
* quantifying chl a from remote sensing data based on relationship between chl a and the ratio of blue to green radiance
* the location of the reflectance peak in 700-720 region shifts to longer wavelengths with increasing concentration of phytoplankton. Sentinel 2 has a peak at 705, can be compared to 665-740 baseline as an indicator of chl a (**magnitude of peak**)

* **Pulliainen et al 2001** used airborne AISA and in situ data from 11 lakes in southern Finland to get chl a, 4 measurement campaigns, 450-900 nm with up to 288 channels, 1m spatial resolution. 
* **Randolph et al 2008** used an ASD Fieldspec spectroradiometer along with in vitro pigment concentrations to test for rapid cyanobacteria bloom assessment, strong relationship between chl a and 665 nm band
* **Wang et al 2009** looked at chl a in Liaodong bay, China from Hyperion with exponential model between ratio of 681 to 671 nm:

$$ 2 \times e^{36.11 \times \frac{R681}{R671}}$$
* Gitelson et al 2011 use HICO (90m resolution) for chl a of the Azov sea: 

$$chl~a = 418.88 \times \bigg((R_{684}~^{-1} - R_{700}~^{-1}) \times R_{720} \bigg) + 19.275$$

* **Awad 2014** used Artificial Neural Networks with Hyperion EO-1 (satellite, 400-2500 nm with 10nm resolution, 30m spatial resolution) data for chl a, in sea water coastal Lebanon (Mediterranean)
* **Toming et al 2016** tested band ratios on Sentinel data. Well-established algorithms for chl a concentration include: height of reflectance peak between 700 and 720
...
* **Bricaud et al 1995** model (used in Kutser et al models) uses power function of chl concentration to get each wavelength's specific chl absorption coefficient: 

$$ a^*_{ph}(\lambda) = A(\lambda)\cdot C_{chl}^{-B(\lambda)}$$
A and B coefficients from lab measurements. This model accounts for packaging effect and means specific absorption coefficient decreases with increasing cell size and cell chl concentration. 

> TODO: Correlations for band ratios
...
* Kutser et al. 1995, 1997, 1998abcd, Eloheimo et al. 1998, Kallio et al. 1998 have used airborne and ship-board optical remote sensing to look at chl a, phaeophytin-a, DOC, TOC, total phosphorus, SM, turbidity, secchi depth in turbid waters of Baltic Sea and Estonian and Finnish lakes. Overall, hard to generalize across sites and seasons. 
* Kutser et al 2001 studied 12 Finnish lakes, coincident measurements included absorption spectra 350-850 nm and vertical profiles of the spectral absorption and attenuation coefficients, spectra above water surface, and AISA airborne imaging spec (7nm resolution, 1 x 1.4m spatial resolution)
* 20m resolution AVIRIS (400-2500 nm, 224 bands) collected over Pantanal in August 1995. **Galvao et al 2003** got field spectra (400-900) of 39 sites across 7 freshwater and 5 saltwater lakes
* **Toming et al 2016** found a good correlation with band ratio algorithms from Sentinel 2 data for 23 sampling points across 9 small and 2 large lakes in Estonia, sampled in August 2015. best performance for CDOM is usually green to red band ratios
* **Kutser et al 2015** showed that iron bound to carbon molecules can absorb light in similar way to CDOM. carbon to iron ratio (and ratio of colored to uncolored DOC) complicate observations of optical effects, actually could make it easier to predict lake DOC than CDOM

CDOM concentration is calculated as: 

$$C_{CDOM} = \frac{c^*_f(\lambda)}{exp(-S(\lambda - \lambda_0))\cdot a^*_{CDOM}(\lambda_0)}$$
* $c^*_f(\lambda)$ is spectrometric reading at ref wavelength (380)
...
* Bukata et al 2004 discuss impacts of inelastic processes

> TODO:

**Phytoplankton fluourescence**
...
* **Ahn et al 1992** measured the backscattering properties of 9 phytoplankters in culture for wavelengths 400-850 nm. Also measured absorption. Chl specific backscattering coefficient is very low, so they conclude that living algae have a negligible influence on backscattering in oceanic waters. 
...
* Gordon et al 2009 show power law variation with wavelength, exponent between 0.4 and 1
