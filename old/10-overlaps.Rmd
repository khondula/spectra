...
```{r}
matches_df <- fs::dir_ls('../neon-sites/results/spectra-matches') %>%
  purrr::map_dfr(~read_csv(.x))
matches_df %>% mutate(abs_diff_days = abs(timediff)) %>%
  write_csv('results/spectra-matches.csv')
matches_df %>% DT::datatable()
```

Note that the location data for AOS has a coordinate uncertainty, approx 10m? 
...
overlap_summary <- all_overlaps_df %>% 
  dplyr::select(aop_siteyear, aq_site, aop_yr, analyte, analyteConcentration, flightdate, radiance_time) %>% 
  group_by(aop_siteyear, aq_site, aop_yr, flightdate, radiance_time, analyte) %>% 
  summarise(avg_conc = mean(analyteConcentration)) %>%
  tidyr::pivot_wider(names_from = analyte, values_from = avg_conc)

overlap_summary %>% DT::datatable()
```

ggtern with TSS, DOC, chl a? 

```{r}
overlap_summary %>%
  ggplot(aes(x = DOC, y = `chlorophyll a`)) +
  geom_point(aes(fill = aq_site), pch = 21, size = 3) +
  scale_y_log10() + scale_x_log10() +
  theme_bw()
overlap_summary %>%
  ggplot(aes(x = DOC, y = TSS+0.1)) +
  geom_point(aes(fill = aq_site), pch = 21, size = 3) +
  scale_y_log10() + scale_x_log10() +
  theme_bw()
overlap_summary %>%
