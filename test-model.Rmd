...
aa <- ggplot(abs_water, aes(x = wl, y = absorb_m1)) + 
  geom_point() +
  scale_y_log10() +
  # geom_vline(aes(xintercept = 700), col = "red") +
  geom_vline(aes(xintercept = my_wavelengths_rs[1])) +
  geom_vline(aes(xintercept = my_wavelengths_rs[100]))
...
backscattering coefficient is HALF of scattering coefficient
bbw = 0.5*bw
...
Packaging effect: see Equation 2.39 (Bricaud et al 1995) 
...
*Slope factor shouldn't be calculated between 254 and 280 but for logistics
assume that absorbance spectra will be determined based on doc and suvas*
...
names(abs_water_list) %>% length()
names(abs_phy) %>% length()
names(abs_cdom) %>% length()

abs_water_list$wl382
abs_phy$wl382
abs_cdom$wl382
...
Aphy_df <- astarPHY_df %>% dplyr::select(nm, abs_phy) %>% 
